...
import com.yanxu.thriftService.student.StudentServiceImpl;
...
@Component
public class StudentThriftServer2 {
    //监听的端口
    @Value("${server.thrift.port}")
    private Integer port;

    //线程池线程数
    @Value("${server.thrift.thread-pool}")
    ...
    public void start()  {
        try {
            ...
            //minWorkerThreads 最小的工作线程2----根据版本而定，低版本没有
            //maxWorkerThreads 最大的工作线程4----同上
            //如果这里Args不使用executorService指定线程池的话，创建THsHaServer会创建一个默认的LinkedBlockingQueue
            THsHaServer.Args arg = new THsHaServer.Args(socket).workerThreads(threadPool);
            //可以自定义指定线程池
